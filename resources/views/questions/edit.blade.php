<x-forum.layouts.app>
  <div class="flex items-center justify-center w-full my-10 px-4">
    <div class="w-full max-w-3xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 shadow-xl rounded-xl p-8 text-white">
      <h2 class="text-3xl font-bold text-center mb-6">Editar Pregunta</h2>
      ...
      <form action="{{ route('questions.update', $question) }}" method="POST" class="space-y-6">
        ...
        <div>
          ...
          <input id="title" name="title" type="text" required value="{{ old('title', $question->title) }}"
        ...
        <div>
          ...
          <textarea id="content" name="content" rows="6" required placeholder="Describe tu duda aquí…" 
                    class="w-full p-3 rounded-lg border border-gray-300 dark:border-gray-700 bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:ring-2 focus:ring-indigo-400 focus:border-indigo-500">{{ old('content', $question->content) }}</textarea>
        ...
        <div class="flex items-center gap-6">
          ...
          <a href="{{ route('question.show', $question) }}" class="px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-700 font-semibold rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-500">
            Cancelar
          </a>
        </div>
      </form>
